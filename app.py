...
def ban_endpoint():
    ...
    try:
        ...
        result["steps"]["1_inspect"] = "started"
        ...
        if 'error' in data:
            result["error"] = f"Token error: {data.get('error')}"
            return jsonify(result), 400
        
        NEW_OPEN_ID = data.get('open_id')
        platform_ = data.get('platform')
        
        result["steps"]["1_inspect"] = {
            "status": "success",
            "open_id": NEW_OPEN_ID,
            "platform": platform_
        }
        ...
        # Step 2: MajorLogin
        result["steps"]["2_majorlogin"] = "started"
        ...
        if not response.ok:
            result["error"] = f"MajorLogin returned error: {response.status_code}"
        ...
        result["steps"]["2_majorlogin"] = {
            "status": "success",
            "account_id": str(resp_msg.account_id),
            "jwt_prefix": resp_msg.account_jwt[:50],
            "key": resp_msg.key.hex(),
            "iv": resp_msg.iv.hex(),
            "timestamp": timetamp
        }
        
        # Step 3: GetLoginData
        result["steps"]["3_getlogindata"] = "started"
        ...
        if r2.status_code != 200:
            result["error"] = f"GetLoginData returned error: {r2.status_code}"
            return jsonify(result), 500
        ...
        try:
            ...
            if json_result:
                ...
                if '14' in parsed_data_login and 'data' in parsed_data_login['14']:
                    ...
                    result["error"] = "Could not find field 14 in parsed data"
                    return jsonify(result), 500
            else:
                result["error"] = "Failed to parse GetLoginData response"
                return jsonify(result), 500
        except Exception as e:
            result["error"] = f"Error processing GetLoginData response: {str(e)}"
            return jsonify(result), 500
        
        result["steps"]["3_getlogindata"] = {
            "status": "success",
            "online_ip": online_ip,
        ...
        result["steps"]["4_build_packet"] = "started"
        ...
        if not final_token_hex:
            result["error"] = "Failed to build start packet"
            return jsonify(result), 500
        
        result["steps"]["4_build_packet"] = {
            "status": "success",
            "packet_length": len(final_token_hex)//2
        }
        
        # Step 5: Connect to game server
        result["steps"]["5_connect_server"] = "started"
        try:
            payload_bytes = bytes.fromhex(final_token_hex)
            response = send_once(online_ip, online_port, payload_bytes, recv_timeout=5.0)
            
            if response:
                result["steps"]["5_connect_server"] = {
                    "status": "success",
                    "response_length": len(response),
                    "response_hex": response.hex()[:100] + "..." if len(response.hex()) > 100 else response.hex()
                }
                result["success"] = True
                result["final_response"] = "✅ Done ban"
            else:
                result["steps"]["5_connect_server"] = {
                    "status": "no_response"
                }
                result["success"] = False
                result["final_response"] = "No response from server"
        except Exception as e:
            result["steps"]["5_connect_server"] = {
                "status": "error",
                "error": str(e)
            ...
            result["success"] = False
            result["final_response"] = f"Connection error: {str(e)}"
    ...
    # Return HTML response with print button
    html_response = f"""
    <!DOCTYPE html>
    <html>
    <head>
        <title>FreeFire Login API</title>
        <style>
            body {{
    ...
    <body>
        <h1>FreeFire Login API Result</h1>
        <div class="result">
